...
- Calendar/task APIs and `/[locale]/app/calendar` page let owners/admins/members create, edit, and delete one-off or finite recurring calendar events, switch event list/day/week/month views, create/assign/edit/delete one-off or recurring tasks, auto-link due tasks to `TASK` calendar events, complete tasks with linked event status updates, and create linked pending expense proposals from tasks.
