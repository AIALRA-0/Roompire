...
- Prisma-backed current session, active household switching, household list, household creation, household settings update including data-retention policy fields and a read-only retention review report, household category/tag management, member list, member role update/removal, invite creation, and invite acceptance APIs.
...
- Browser UI can switch dev users, switch the active household, create households, edit household settings plus active categories/tags, review current data-retention impact without deleting records, create invite codes/links, accept invite codes or tokenized links, open member directory pages, update member roles, and remove members.
...
- Cross-currency proposals can omit `fxRate` under the default household FX policy; Roompire locks the expense-date rate from the `FxRate` cache or configured FX provider and shows whether the copied lock came from manual entry, provider/cache lookup, same-currency handling, or original-currency debt. Live provider successes/failures after cache misses are recorded for owner/admin ops visibility. Households can switch to manual FX approval, which requires a reviewed `fxRate` for every cross-currency proposal.
...
- Owner/admin ledger correction APIs support manual adjustments, FX-difference adjustments, and reversal of unallocated open obligations with append-only ledger transactions.
