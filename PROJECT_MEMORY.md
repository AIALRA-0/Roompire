...
- Task board views slice implemented on branch `feat/task-board-views`: the localized calendar workspace now lets users switch tasks between full-action list, status board, and due-date calendar views derived from the loaded task window, with desktop/mobile E2E and live-domain validation.
- Calendar/task work filters slice implemented on branch `feat/calendar-work-filters`: calendar event APIs now filter by type/status/member/template category, task APIs filter by status/priority/assignee/category, the localized calendar page keeps filters in the URL, tasks can store/edit/display category, and desktop/mobile E2E plus live-domain validation cover the server-backed filters.
- Expense proposal filters slice implemented on branch `feat/expense-proposal-filters`: expense proposal listing now filters by text/status/category/tag/member/date/visible amount with cursor validation inside the filtered window, `/[locale]/app` keeps proposal queue filters in the URL, dashboard load-more preserves active filters, docs/OpenAPI/i18n are updated, and desktop/mobile E2E plus real-domain smoke/browser validation cover the path.
...
- Database branch CI trigger slice implemented on branch `chore/ci-db-branch-triggers`: CI and E2E push triggers now include `db/**` so migration/index branches receive the same automated quality gates as feature, ops, and chore branches.
...
- Browser workspace auto-cleanup slice implemented on branch `ops/browser-workspace-auto-cleanup`: `server_housekeeping.sh` supports `ROOMPIRE_HOUSEKEEPING_CLEAN_BROWSER_WORKSPACES=auto`, which removes old Codex browser workspace dependency/build/test-output artifacts outside the current checkout only when root free bytes fall below the configured threshold and no active build/test process is detected; the installed housekeeping timer now uses this mode.
- Docker image retention slice implemented on branch `ops/docker-space-housekeeping`: `server_housekeeping.sh` prunes unused one-shot Roompire image repositories, defaulting to `roompire-migrator`, only after confirming no current or stopped container still references the image; the installed housekeeping service enables this cleanup alongside build-cache pruning.
...
- Smoke freshness warning slice implemented on branch `ops/smoke-freshness-warning`: `/api/v1/ops/status` now raises `smoke_stale` when the latest passed/failed production smoke result is older than `ROOMPIRE_SMOKE_STATUS_STALE_MS` (default two hours), and the localized ops page surfaces the warning so an old green smoke file cannot mask broken smoke automation.
- Production container health slice implemented on branch `ops/container-health-status`: `collect_ops_status.sh` records configured Compose container running/health/restart state, `/api/v1/ops/status` exposes `containerHealth`, and `/[locale]/app/ops` surfaces stopped, unhealthy, starting, or unreadable production containers without giving the web container Docker socket access.
- Restore drill status slice implemented on branch `ops/restore-drill-status`: non-destructive PostgreSQL restore verification now writes `ops/status/latest-restore-drill.json`, the host snapshot/API/page surface the latest backup artifact and audit hash counts, and stale/missing/failed restore proof raises owner-visible ops warnings.
- Shared app storage inventory slice implemented on branch `ops/shared-app-storage-inventory`: `collect_shared_app_storage.sh` writes a separate slow-scan status file for shared-server app directories, `collect_ops_status.sh` reads it without running the scan during the 15-minute refresh, `/api/v1/ops/status` and `/[locale]/app/ops` show shared app storage rows/timeouts, and stale or partial recorded inventories raise owner-visible warnings.
- Disk capacity trend slice implemented and deployed on branch `ops/disk-capacity-trends`: `collect_ops_status.sh` appends root filesystem samples to ignored `ops/status/disk-history.json`, folds a bounded disk trend into `/api/v1/ops/status`, and `/[locale]/app/ops` displays sample count, trend window, available-space change, used-per-day rate, and projected-full status while suppressing projected-full warnings until the configured minimum history window is reached.
- Docker image inventory slice implemented and deployed on branch `ops/docker-image-inventory`: `collect_ops_status.sh` records the largest Docker images by size with image ID, reference, container count, and created time, `/api/v1/ops/status` exposes `dockerImageInventory`, and `/[locale]/app/ops` displays the top images without pruning shared server images.
- Docker safe-cleanup classification slice implemented and deployed on branch `ops/docker-reclaimable-safety`: Docker storage ops now separates Docker-reported reclaimable bytes from conservative safe cleanup candidates, warning only on build cache plus dangling or configured one-shot Roompire images with no container references while keeping active shared images and volumes as evidence.
- Root storage inventory slice implemented and deployed on branch `ops/root-storage-inventory`: `collect_ops_status.sh` records a bounded, timeout-protected host path size inventory, `/api/v1/ops/status` exposes `rootStorageInventory`, and `/[locale]/app/ops` shows the largest recorded root storage sources such as `/var/lib/containerd` without granting the web container host command privileges or pruning shared app directories.
...
| 2026-07-06 | Honor household approval policies at maturity                | The default keeps per-share maturity, `ALL_PARTICIPANTS` prevents ledger impact until every participant share is approved, and `PAYER_ONLY` lets the primary payer confirm shares without debtor approve buttons.   |
...
| 2026-07-06 | Run CI and E2E on db branches                                | Database migrations and indexes affect production safety, so `db/**` branches should get the same push-time CI/E2E coverage as feature and ops branches.                                                            |
...
| 2026-07-06 | Remove unused one-shot Roompire images in housekeeping       | The migrator image is rebuilt before deployment migrations and is not needed after `run --rm`; pruning it only when no container references it recovers space without touching the live web image or data volumes.  |
| 2026-07-06 | Surface production container health from host snapshots      | The self-hosted server can inspect Compose containers during scheduled ops refresh, but the web app should consume only read-only JSON instead of receiving Docker socket or host command access.                   |
| 2026-07-06 | Surface restore drill proof from host snapshots              | Encrypted backups are only trustworthy if the latest non-destructive restore result, verified backup artifact, and audit hash-chain counts are visible from the owner ops page before a real recovery is needed.    |
| 2026-07-06 | Split slow shared app storage scans from fast ops refresh    | Broad shared-server directories can take minutes to scan, so they should be collected by a separate low-frequency timer and read from JSON by the normal ops snapshot instead of blocking 15-minute health refresh. |
...
3. Continue root-disk capacity planning; disk trend, Docker image inventory, safe-cleanup classification, fast root storage inventory, shared app storage inventory, and old browser-workspace generated-artifact auto-cleanup are live, but the current server still ends around 6-8GB free / 97% used. The latest shared app inventory records about 30.2GB from scanned `/srv/aialra/apps` children and times out on `/srv/aialra/apps/opencode-turn-engine`, so clearing the warning likely needs a larger disk or explicit shared-app/shared-image retention policy.
...
- 2026-07-06 Household data retention settings:
  ...
  - Owner/admin household settings UI and `PATCH /api/v1/households/{householdId}` now read/write retention values, household/session/list responses serialize them, and `household.settings_updated` audit events include before/after retention values. Automatic deletion is intentionally not triggered by these settings because ledger, audit hash-chain, receipt, and evidence deletion semantics need a separate worker/design.
  - Updated en-US/zh-CN copy, README, database/API/backlog docs, OpenAPI, and Playwright coverage. The E2E owner-settings flow now saves `365`/`1095`, verifies API response fields, and checks invalid `29` days returns `400`.
  - Local verification passed: `pnpm db:validate`, `pnpm db:generate`, `pnpm db:migrate`, JSON parse for en-US/zh-CN messages, OpenAPI YAML parse, `pnpm typecheck`, `pnpm lint`, `pnpm test` (12 files, 50 tests), `pnpm format:check`, `git diff --check`, `pnpm build`, targeted `ROOMPIRE_E2E_PORT=3118 pnpm e2e --grep "owner updates settings and manages a linked invitee"` (desktop/mobile, 2 passed), targeted retry coverage for the two stabilized mutation helpers (4 passed), and full `ROOMPIRE_E2E_PORT=3121 pnpm e2e` (50 passed across Chromium desktop/mobile).
  - Production deployment was performed directly on the self-hosted server without SSH: safe housekeeping recovered root headroom, Docker Compose built `web` and `migrate`, `prisma migrate deploy` applied `20260706030000_add_household_data_retention_settings`, `up -d web` recreated `roompire-web-1`, and Postgres/Redis/web returned healthy on `127.0.0.1:18300`.
  - Production DB verification confirmed both CHECK constraints exist. Real-domain smoke passed for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`.
  - Live validation through the site gate created a temporary household on `https://roompire.aialra.online`, updated retention values from the real settings UI, verified the API response and persisted UI values, confirmed the production audit event stored `365|1095`, saved `output/playwright/roompire-retention-settings-live-20260706124624.png`, then deleted the temporary household and confirmed zero `Live Retention %` households remained.
  - Post-deploy housekeeping removed the one-shot migrator image and Docker build cache; final smoke still passed, ops status reported container health/latest smoke/Docker storage/backup freshness OK with expected warnings for high disk use, depleting disk trend, missing passphrase escrow, and disabled offsite backup; root ended around 8.3GB free / 96% used with Docker build cache at `0B`.
- 2026-07-06 Household approval policy enforcement:
  - Branch: `feat/approval-policy-enforcement`.
  - Proposal serialization now exposes `approvalPolicy`; share approval enforces `PAYER_AND_EACH_DEBTOR`, `ALL_PARTICIPANTS`, and `PAYER_ONLY`; payer-only detail pages show primary-payer confirmation controls; all-participants approvals stay out of the ledger until the final share is approved; rejection/request-changes remain debtor share actions.
  - Updated en-US/zh-CN proposal detail copy, OpenAPI, API/technical docs, README/backlog, and Playwright desktop/mobile coverage for all-participants delayed maturity.
  - Verification passed before deployment: `pnpm db:validate`, JSON parse for en-US/zh-CN messages, OpenAPI YAML parse, `pnpm typecheck`, `pnpm lint`, `pnpm test` (12 files, 50 tests), `git diff --check`, `pnpm format:check`, targeted `ROOMPIRE_E2E_PORT=3116 pnpm e2e --grep "all-participants approval waits"` (desktop/mobile, 2 passed), `pnpm build`, and full `ROOMPIRE_E2E_PORT=3117 pnpm e2e` (50 passed across Chromium desktop/mobile).
  - Production deployment was performed directly on the self-hosted server without SSH: safe housekeeping recovered root headroom, Docker Compose built `web` and `migrate`, `prisma migrate deploy` reported no pending migrations, `up -d web` recreated `roompire-web-1`, and Postgres/Redis/web returned healthy on `127.0.0.1:18300`.
  - Real-domain smoke passed for `https://roompire.aialra.online`: `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`.
  - Live validation through the site gate created an isolated temporary payer-only household and synthetic member, opened the real proposal detail page in Playwright, verified the payer-only hint, approved a share from the primary-payer UI, confirmed `MATURED_TO_LEDGER` plus a live `ledgerObligationId`, saved `output/playwright/roompire-approval-policy-live-20260706125553.png`, and cleaned temporary production rows with zero remaining validation households/users.
  - Post-deploy housekeeping removed build cache/one-shot migrator residue, smoke still passed, ops status reported container health and latest smoke OK with expected disk/backup escrow/offsite warnings, Docker build cache at `0B`, and root around 9.3GB free / 96% used.
- 2026-07-06 Shared app storage inventory:
  - Branch: `ops/shared-app-storage-inventory`.
  - Added `scripts/collect_shared_app_storage.sh`, which scans immediate children of configured shared app roots with per-path and total timeouts and writes `ops/status/shared-app-storage.json`; `roompire-shared-app-storage.timer` runs it separately from the normal 15-minute ops refresh.
  - `scripts/collect_ops_status.sh` now folds that status file into `sharedAppStorageInventory`; `/api/v1/ops/status` normalizes it, derives `shared_app_storage_attention` and `shared_app_storage_stale`, and the localized ops page shows recorded shared app rows plus timed-out paths. OpenAPI, E2E fixtures/assertions, env examples, systemd templates, backlog, and deployment runbook were updated.
  - Real server evidence: a broad `/srv/aialra/apps` scan exceeded 100 seconds, and `/srv/aialra/apps/opencode-turn-engine` alone exceeded a 120-second test timeout. The separate collector recorded 12 rows totaling about 30.2GB and correctly flagged `opencode-turn-engine` as timed out.
  - Local verification passed: `bash -n scripts/collect_shared_app_storage.sh scripts/collect_ops_status.sh`, en-US/zh-CN message JSON parse, OpenAPI YAML parse via `python3`, `git diff --check`, `systemd-analyze verify`, `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (12 files, 50 tests), `pnpm build`, and targeted `ROOMPIRE_E2E_PORT=3113 pnpm e2e --grep "owner reviews ops health and viewer is blocked"` on Chromium desktop/mobile.
  - Installed host systemd units `/etc/systemd/system/roompire-shared-app-storage.service` and `.timer`, adjusted the live `WorkingDirectory`, added the shared app status file env to `/etc/systemd/system/roompire-ops-status.service`, verified units, reloaded systemd, enabled the daily timer for `2026-07-07 03:35:00 CEST`, and refreshed ops status successfully.
  - Production deployment was performed directly on the self-hosted server without SSH. `docker compose --env-file .env.production -f docker-compose.prod.yml -f docker-compose.nginx.example.yml build web` rebuilt `roompire-web:latest`, `up -d web` recreated the live web container, and Docker health returned `healthy`.
  - Real-domain verification passed: `./scripts/smoke_production.sh` returned 200 for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`; authenticated `/api/v1/ops/status` returned `sharedAppStorageInventory.status=warning`, 12 recorded rows, timed-out path `/srv/aialra/apps/opencode-turn-engine`, latest smoke `passed`, and container health `ok`.
  - Authenticated real-browser validation opened `https://roompire.aialra.online/en-US/app/ops` on desktop and mobile, verified the shared app storage section shows `Attention`, recorded size around 30.2GB, timed-out paths `1`, first row `/srv/aialra/apps/codex-turn-engine`, zero horizontal overflow, and zero console/page errors; screenshots were saved under `output/playwright/roompire-shared-app-storage-live-*.png`.
  - Post-deploy housekeeping reclaimed 1.798GB of Docker build cache, skipped workspace artifact cleanup because an unrelated Readlayer Playwright process was active, refreshed ops status, and left root disk around 7.7GB free / 97% used. Current expected warnings are `disk_high_usage`, `shared_app_storage_attention`, and `backup_offsite_disabled`.
- 2026-07-06 Restore drill status:
  - Branch: `ops/restore-drill-status`.
  - `scripts/verify_postgres_backup.sh` now writes `ops/status/latest-restore-drill.json` for passed/failed restore drills, including backup artifact path, temporary drill database, audit event total, hashed count, broken count, and message.
  - `scripts/collect_ops_status.sh` folds that file into the host ops snapshot; `/api/v1/ops/status` normalizes it, derives `restore_drill_failed`, `restore_drill_missing`, and `restore_drill_stale` warnings, and the localized ops page displays the latest restore drill in the backup card. OpenAPI, E2E fixtures/assertions, env examples, systemd units, backlog, security/backup docs, and deployment runbook were updated.
  - Local verification passed: `bash -n scripts/verify_postgres_backup.sh scripts/collect_ops_status.sh`, en-US/zh-CN message JSON parse, OpenAPI YAML parse, `git diff --check`, `systemd-analyze verify`, `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (12 files, 50 tests), `pnpm build`, and targeted `ROOMPIRE_E2E_PORT=3112 pnpm e2e --grep "owner reviews ops health and viewer is blocked"` on Chromium desktop/mobile.
  - A real encrypted restore drill passed on the self-hosted server against `/srv/aialra/backups/roompire/postgres/roompire_20260706T042535Z.dump.enc`, restoring into a temporary database and verifying `audit_total=12`, `audit_hashed=12`, `audit_broken=0`.
  - Installed host systemd service environment was updated for `roompire-backup.service` and `roompire-ops-status.service`, verified with `systemd-analyze verify`, reloaded with `systemctl daemon-reload`, and `roompire-ops-status.service` completed successfully.
  - Production deployment was performed directly on the self-hosted server without SSH. Housekeeping recovered root headroom, `docker compose --env-file .env.production -f docker-compose.prod.yml -f docker-compose.nginx.example.yml build web` rebuilt `roompire-web:latest`, `up -d web` recreated the live web container, and Docker health returned `healthy`.
  - Real-domain verification passed: `./scripts/smoke_production.sh` returned 200 for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`; authenticated `/api/v1/ops/status` returned `latestRestoreDrill.status=passed`, latest smoke `passed`, audit counts `12/12/0`, and expected warnings only for disk pressure plus disabled offsite backup.
  - Authenticated real-browser validation opened `https://roompire.aialra.online/en-US/app/ops` on desktop and mobile, verified restore drill `Passed`, latest encrypted backup path, broken hashes `0`, zero horizontal overflow, and zero console/page errors, then saved screenshots under `output/playwright/roompire-restore-drill-live-*.png`.
- 2026-07-06 Production container health status:
  - Branch: `ops/container-health-status`.
  - `scripts/collect_ops_status.sh` now records `containerHealth` for configured production Compose containers, defaulting to `roompire-web-1 roompire-postgres-1 roompire-redis-1`, with running state, Docker health, restart count, image, start/finish timestamps, and per-container/top-level health status.
  - `/api/v1/ops/status` normalizes the new field, derives `container_health_unknown` / `container_health_attention` summary warnings, and the localized ops page shows a production containers section under Docker storage. OpenAPI, E2E fixtures/assertions, env examples, backlog, and deployment runbook were updated.
  - Local verification passed: `bash -n scripts/collect_ops_status.sh`, real host `./scripts/collect_ops_status.sh`, en-US/zh-CN message JSON parse, OpenAPI YAML parse via PyYAML, `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (12 files, 50 tests), `pnpm build`, and targeted `ROOMPIRE_E2E_PORT=3111 pnpm e2e --grep "owner reviews ops health and viewer is blocked"` on Chromium desktop/mobile.
  - Production deployment was performed directly on the self-hosted server without SSH. Housekeeping first recovered root headroom, then `docker compose --env-file .env.production -f docker-compose.prod.yml -f docker-compose.nginx.example.yml build web` rebuilt `roompire-web:latest`, `up -d web` recreated the live web container, and Docker health returned `healthy`.
  - Real-domain verification passed: `./scripts/smoke_production.sh` returned 200 for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`; authenticated `/api/v1/ops/status` returned `containerHealth.status=ok` with web/Postgres/Redis all `healthy`, latest smoke `passed`, and expected warnings for disk pressure plus disabled offsite backup.
  - Authenticated real-browser validation opened `https://roompire.aialra.online/en-US/app/ops` on desktop and mobile, verified the production containers section includes `roompire-web-1` and `healthy`, found zero horizontal overflow and zero console/page errors, and saved screenshots under `output/playwright/roompire-container-health-live-*.png`.
  - Post-deploy housekeeping reclaimed Docker build cache and refreshed ops status; root disk ended around 5.4GB free / 98% used with `containerHealth` still `ok`.
- 2026-07-06 Systemd current-checkout installer:
  - Branch: `ops/systemd-current-checkout-installer`.
  - Added `scripts/install_systemd_units.sh`, a dry-run-by-default installer that renders selected or all `ops/systemd/roompire-*` units with the current checkout as `WorkingDirectory`, rewrites template `/srv/roompire/current` references including `EnvironmentFile`, verifies rendered units with `systemd-analyze verify`, refuses non-Roompire unit names, and copies to `/etc/systemd/system/` only when `ROOMPIRE_SYSTEMD_INSTALL_CONFIRM=install`.
  - Updated `docs/14_deployment_runbook.md` so ops automation, housekeeping, reminders/shared-app-storage, and backup timers use the installer instead of manual `cp` plus hand-edited paths. This reflects the current self-hosted server workflow: operate directly on this machine, without SSH to a separate VPS.
  - Verification passed: `bash -n scripts/install_systemd_units.sh`, full dry-run rendering 12 units, special-character working-directory dry-run, temporary-target confirmed install with no `/srv/roompire/current` leftovers and the reminders `EnvironmentFile` pointing at this checkout, `systemd-analyze verify /etc/systemd/system/roompire-*.service /etc/systemd/system/roompire-*.timer`, `git diff --check`, `pnpm format:check`, `pnpm lint`, and `pnpm typecheck`.
  - Production action was performed directly on the self-hosted server without SSH: installed all 12 rendered units to `/etc/systemd/system/`, enabled the six Roompire timers, started only `roompire-ops-status.service` and `roompire-smoke.service` for low-risk validation, confirmed both ended with `Result=success` / `ExecMainStatus=0`, and verified installed unit files no longer contain `/srv/roompire/current`.
  - Real-domain validation passed: `ops/status/latest-smoke.json` recorded passed checks for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest` on `https://roompire.aialra.online`; authenticated `/api/v1/ops/status` returned `opsStatusService.status=ok`, `smokeService.status=ok`, `backupTimer.status=ok`, `housekeepingTimer.status=ok`, `reminderTimer.status=ok`, `backupFreshness.status=ok`, with expected remaining warnings for disk pressure, missing backup passphrase escrow proof, and disabled offsite backup.
- 2026-07-06 Backup freshness status:
  - Branch: `ops/backup-freshness-status`.
  - Added `backupFreshness` to the host ops snapshot, `/api/v1/ops/status`, OpenAPI, localized ops UI, env examples, systemd ops-status unit, runbook/security docs, backlog status, and targeted Playwright fixture. The snapshot checks the latest complete local backup set across PostgreSQL dump, upload-volume archive, and file manifest artifacts, defaulting to a 36-hour freshness window via `ROOMPIRE_BACKUP_FRESHNESS_STALE_MS`.
  - Summary warnings now include `backup_freshness_attention` when any backup category is missing or stale and `backup_freshness_unknown` when the backup root cannot be read. Backup freshness is separate from encryption health so a timer/service success cannot hide a missing category.
  - Local verification passed: `bash -n`, en-US/zh-CN JSON parse, OpenAPI YAML parse, `systemd-analyze verify`, temporary collector drills for complete and missing backup sets, `git diff --check`, `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (12 files, 50 tests), `pnpm build`, and targeted `ROOMPIRE_E2E_PORT=3115 pnpm e2e --grep "owner reviews ops health and viewer is blocked"` (desktop/mobile, 2 passed).
  - Production deployment was performed directly on the self-hosted server without SSH: installed the updated `/etc/systemd/system/roompire-ops-status.service` with the real current-checkout `WorkingDirectory`, verified/reloaded systemd, confirmed `roompire-ops-status.service` runs with `Result=success` / `ExecMainStatus=0`, rebuilt `roompire-web:latest`, recreated `roompire-web-1`, and confirmed web/Postgres/Redis healthy on `127.0.0.1:18300`.
  - Real-domain verification passed: `./scripts/smoke_production.sh` returned 200 for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`; authenticated `/api/v1/ops/status` returned `backupFreshness.status=ok`, latest complete backup `2026-07-06T04:25:35.000Z`, Docker storage `ok`, latest smoke `passed`, and expected warnings `disk_high_usage`, `backup_passphrase_escrow_missing`, and `backup_offsite_disabled`.
  - Authenticated real-browser validation opened `https://roompire.aialra.online/en-US/app/ops` on desktop and mobile, verified the new backup freshness `OK` section, confirmed no horizontal overflow, and saved screenshots under `output/playwright/roompire-ops-backup-freshness-live-*.png`.
  - Post-deploy housekeeping reclaimed about 2.568GB of Docker build cache, refreshed ops status, left Docker build cache at `0B`, and ended around 9.7GB free / 95% used.
- 2026-07-06 Backup passphrase escrow visibility:
  - Branch: `ops/backup-passphrase-escrow-status`.
  - Added non-secret backup passphrase escrow proof metadata: `scripts/write_backup_passphrase_escrow_status.sh` refuses to write unless `ROOMPIRE_BACKUP_PASSPHRASE_ESCROW_CONFIRM=recorded`, records method/custodian/verification timestamp only, and never reads, prints, writes, or stores the passphrase itself.
  - `scripts/collect_ops_status.sh`, `OpsStatusSnapshot`, `/api/v1/ops/status`, OpenAPI, localized en-US/zh-CN Ops UI, env examples, systemd ops-status unit, runbook/security docs, and the targeted Playwright ops fixture now expose `backupPassphraseEscrow`. When backup encryption is enabled, the summary raises `backup_passphrase_escrow_missing` until proof metadata exists, or `backup_passphrase_escrow_attention` when the proof is stale/invalid.
  - Local verification passed: `bash -n scripts/collect_ops_status.sh scripts/write_backup_passphrase_escrow_status.sh`, JSON parse for en-US/zh-CN messages, `git diff --check`, `systemd-analyze verify`, temporary escrow writer/collector drill for present and missing status files, `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (12 files, 50 tests), `pnpm build`, and targeted `ROOMPIRE_E2E_PORT=3114 pnpm e2e --grep "owner reviews ops health and viewer is blocked"` (desktop/mobile, 2 passed).
  - Production deployment was performed directly on the self-hosted server without SSH: updated `/etc/systemd/system/roompire-ops-status.service`, refreshed host ops status, rebuilt `roompire-web:latest`, recreated `roompire-web-1`, confirmed web/Postgres/Redis healthy on `127.0.0.1:18300`, ran real-domain smoke for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`, and refreshed ops status again.
  - Real-domain verification confirmed authenticated `/api/v1/ops/status` returns `backupEncryption.status=ok`, `backupPassphraseEscrow.configured=false`, `backupPassphraseEscrow.status=warning`, and `latestSmoke.status=passed`. Authenticated desktop/mobile browser checks opened `https://roompire.aialra.online/en-US/app/ops`, verified the passphrase escrow `Attention` state and missing-proof warning, captured `output/playwright/roompire-ops-passphrase-escrow-live*.png`, and confirmed no horizontal overflow.
  - Post-deploy housekeeping reclaimed about 2.739GB of Docker build cache, refreshed ops status, left Docker build cache at `0B`, and ended around 11GB free / 95% used. Expected remaining warnings are `disk_high_usage`, `backup_passphrase_escrow_missing`, and `backup_offsite_disabled`; the escrow warning should remain until a human confirms the passphrase has been escrowed outside this server and runs the writer script.
- 2026-07-06 Smoke freshness warning:
  - Branch: `ops/smoke-freshness-warning`.
  - `/api/v1/ops/status` now derives a `smoke_stale` summary warning when a passed or failed latest production smoke result is older than `ROOMPIRE_SMOKE_STATUS_STALE_MS`, defaulting to two hours.
  - The localized ops page maps `smoke_stale` to en-US/zh-CN warning text, and `.env.example`, `.env.production.example`, plus `docs/14_deployment_runbook.md` document the threshold.
  - Local verification passed: JSON parse for en-US/zh-CN messages, `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (12 files, 50 tests), `pnpm build`, and targeted `ROOMPIRE_E2E_PORT=3110 pnpm e2e --grep "owner reviews ops health and viewer is blocked"` (desktop/mobile, 2 passed). The E2E fixture proves a fresh host snapshot with only an old latest-smoke timestamp returns `summary={status:"warning", warnings:["smoke_stale"]}` and renders the stale-smoke warning on the ops page.
  - Production deployment was performed directly on the self-hosted server without SSH: current-checkout build artifacts were cleaned first to recover disk headroom, `docker compose --env-file .env.production -f docker-compose.prod.yml -f docker-compose.nginx.example.yml build web` rebuilt `roompire-web:latest`, and `up -d web` recreated the live web container, which became healthy.
  - Real-domain verification passed: `./scripts/smoke_production.sh` returned 200 for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`; authenticated `/api/v1/ops/status` showed latest smoke `passed` with no `smoke_stale` warning. Post-deploy `roompire-housekeeping.service` succeeded with `ExecMainStatus=0`, refreshed ops status, and left expected warnings `disk_high_usage` and `backup_offsite_disabled`.
  - Authenticated real-browser validation opened `https://roompire.aialra.online/en-US/app/ops` on desktop and mobile, verified ops summary/smoke content, confirmed the fresh smoke result does not show the stale-smoke warning, found zero horizontal overflow, and saved screenshots under `output/playwright/roompire-smoke-freshness-live-ops-*.png`.
- 2026-07-06 Browser workspace auto-cleanup:
  - Branch: `ops/browser-workspace-auto-cleanup`.
  - `scripts/server_housekeeping.sh` now supports `ROOMPIRE_HOUSEKEEPING_CLEAN_BROWSER_WORKSPACES=auto` plus `ROOMPIRE_HOUSEKEEPING_WORKSPACE_ARTIFACT_MIN_AVAILABLE_BYTES`; it removes only generated dependency/build/test-output directories outside the current checkout when root free bytes are below the threshold and no active Next/Playwright/Turbo/pnpm build or test process is detected.
  - `ops/systemd/roompire-housekeeping.service` now enables browser-workspace auto cleanup at a 10GiB free-space threshold. The installed `/etc/systemd/system/roompire-housekeeping.service` was updated, verified with `systemd-analyze verify`, reloaded with `systemctl daemon-reload`, and started successfully with `Result=success` / `ExecMainStatus=0`.
  - Manual dry-run showed old Babbledeck/Readlayer generated artifacts as candidates while skipping current Roompire checkout artifacts above the 6GiB threshold. Real cleanup removed old workspace generated artifacts outside the current checkout; follow-up `find` showed only the current Roompire checkout's `.turbo` and `node_modules` remain among browser-workspace generated directories.
  - `ctr` inspection showed `/var/lib/containerd` is active Docker/containerd `moby` state with active shims, images, containers, and snapshots, so it remains evidence for capacity planning rather than a direct cleanup target.
  - Verification passed: `bash -n scripts/server_housekeeping.sh`, `git diff --check`, `pnpm format:check`, `systemd-analyze verify`, manual housekeeping dry-run, real housekeeping cleanup, `systemctl start roompire-housekeeping.service`, and authenticated real-domain `/api/v1/ops/status` check. Ops status now reports housekeeping success and root storage inventory OK; remaining summary warnings are `disk_high_usage` and `backup_offsite_disabled`.
- 2026-07-06 Root storage inventory:
  - Branch: `ops/root-storage-inventory`.
  - `scripts/collect_ops_status.sh` now records `rootStorageInventory` with configurable path list, row limit, and timeout. The default fast inventory avoids broad shared-server app scans and records high-signal paths such as `/var/lib/containerd`, `/var/lib/docker`, `/var/lib/snapd`, `/var/log`, `/var/cache`, Codex app state, Roompire backups, and `/home`.
  - `/api/v1/ops/status`, OpenAPI, localized en-US/zh-CN ops UI, and the Playwright ops fixture now expose and test root storage source rows. `ops/systemd/roompire-ops-status.service` documents the 60s collection cap for the host timer.
  - Manual capacity evidence found `/srv/aialra/apps` around 53GB, including `opencode-turn-engine` around 27GB and `codex-turn-engine` around 7.2GB; these are shared-server app directories and are not included in unattended 15-minute ops snapshots until their scan cost/retention policy is explicit.
  - Local verification passed: `bash -n scripts/collect_ops_status.sh`, JSON parse for en-US/zh-CN messages, OpenAPI YAML parse, `git diff --check`, `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (12 files, 50 tests), `pnpm build`, and targeted `ROOMPIRE_E2E_PORT=3108` plus `ROOMPIRE_E2E_PORT=3109 pnpm e2e --grep "owner reviews ops health and viewer is blocked"` (desktop/mobile, 2 passed each run).
  - Production deployment from the self-hosted server used Docker Compose only, without SSH: built `roompire-web:latest`, recreated `roompire-web-1`, confirmed Postgres/Redis/web healthy on `127.0.0.1:18300`, ran real-domain smoke for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`, and refreshed the host ops snapshot.
  - Real-domain verification confirmed authenticated `/api/v1/ops/status` returns `rootStorageInventory.status=ok`, `timeoutMs=60000`, recorded size around 38.5GB, first path `/var/lib/containerd` around 27.8GB, `dockerStorage.status=ok`, Docker safe cleanup `0B`, and summary warnings remain `disk_high_usage` plus `backup_offsite_disabled`. Real browser desktop/mobile checks opened `/en-US/app/ops`, verified the storage source card, captured `output/playwright/roompire-root-storage-live-*`, observed zero console errors, and confirmed no mobile horizontal overflow.
  - Post-deploy housekeeping removed unused `roompire-migrator:latest`, reclaimed 4.085GB of Docker build cache, refreshed ops status, and left production around 7.7GB free / 97% used with Docker build cache at `0B`.
- 2026-07-06 Docker safe-cleanup classification:
  - Branch: `ops/docker-reclaimable-safety`.
  - `scripts/collect_ops_status.sh` now records `dockerStorage.safeReclaimableBytes` and `unsafeReclaimableBytes`; summary warnings use conservative safe cleanup candidates instead of raw `docker system df` image/volume reclaimable totals. Safe candidates are build cache plus dangling images or configured one-shot Roompire images with zero container references.
  - `dockerImageInventory` now records active image bytes, inactive image bytes, safe image candidate bytes, and candidate image rows. The ops page shows safe cleanup, Docker-reported reclaimable, active/inactive image size, and safe image candidates.
  - Local verification passed: `bash -n scripts/collect_ops_status.sh`, JSON parse for en-US/zh-CN messages, OpenAPI YAML parse, `pnpm format:check`, `git diff --check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (12 files, 50 tests), `pnpm build`, and targeted `ROOMPIRE_E2E_PORT=3107 pnpm e2e --grep "owner reviews ops health and viewer is blocked"` (desktop/mobile, 2 passed).
  - Production deployment from the self-hosted server used Docker Compose only, without SSH: built `roompire-web:latest`, recreated `roompire-web-1`, confirmed Postgres/Redis/web healthy on `127.0.0.1:18300`, ran real-domain smoke for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`, and refreshed the host ops snapshot.
  - Real-domain verification confirmed authenticated `/api/v1/ops/status` returns `dockerStorage.status=ok`, Docker reported reclaimable around 33GB, safe cleanup around 245MB after housekeeping, all image bytes active, zero safe image candidates, and summary warnings reduced to `disk_high_usage` plus `backup_offsite_disabled`. Real browser desktop/mobile checks opened `/en-US/app/ops`, verified the Docker safe-cleanup card values, captured `output/playwright/roompire-docker-safe-cleanup-live-*`, observed zero console errors, and confirmed no mobile horizontal overflow.
  - Post-deploy housekeeping reclaimed 1.79GB of Docker build cache, refreshed ops status, and ended with root disk around 8.0GB free / 96% used. Docker build cache remained at about 234MB reported size but `0B` Docker-reported reclaimable, below the safe-cleanup warning threshold.
- 2026-07-06 Docker image inventory:
  - Branch: `ops/docker-image-inventory`.
  - Added read-only Docker image inventory collection to `scripts/collect_ops_status.sh`; the host snapshot now includes `dockerImageInventory` with configurable top-image limit, total image bytes, image references, image IDs, container counts, sizes, created times, status, and errors.
  - `/api/v1/ops/status`, OpenAPI, localized en-US/zh-CN ops UI, and the Playwright ops fixture now expose and test the largest-image inventory. The ops page also received a mobile width fix so long image references truncate instead of widening the page.
  - Local verification passed: `bash -n scripts/collect_ops_status.sh`, JSON parse for en-US/zh-CN messages, OpenAPI YAML parse, `pnpm format:check`, `git diff --check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (12 files, 50 tests), `pnpm build`, and targeted `ROOMPIRE_E2E_PORT=3107 pnpm e2e --grep "owner reviews ops health and viewer is blocked"` (desktop/mobile, 2 passed).
  - Production deployment from the self-hosted server used Docker Compose only, without SSH: built `roompire-web:latest`, recreated `roompire-web-1`, confirmed Postgres/Redis/web healthy on `127.0.0.1:18300`, ran real-domain smoke for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`, and refreshed the host ops snapshot.
  - Real-domain verification confirmed authenticated `/api/v1/ops/status` returns `dockerImageInventory.status=ok`, `topLimit=8`, total image bytes around 28.8GB, and top images led by `aialra/pdf-onlyoffice:local`, `lscr.io/linuxserver/chromium:latest`, and `docker.openhands.dev/openhands/openhands:1.7`. Real browser desktop/mobile checks opened `/en-US/app/ops`, verified the largest-image rows, captured final screenshots under `output/playwright/roompire-docker-image-inventory-live-final-*`, observed zero console errors, and confirmed the mobile card/inventory/row geometry stays inside a 390px viewport.
  - Post-deploy housekeeping reclaimed 2.2GB of Docker build cache, refreshed ops status, left Docker build cache at `0B`, and ended with root disk around 8.3GB free / 96% used. Expected remaining warnings are `disk_high_usage`, `docker_reclaimable_high`, and `backup_offsite_disabled`.
- 2026-07-06 Disk capacity trend:
  - Branch: `ops/disk-capacity-trends`.
  - Added disk history persistence to `scripts/collect_ops_status.sh`, with ignored `ops/status/disk-history.json`, configurable sample retention, minimum projection window, and projected-full warning window.
  - `/api/v1/ops/status` and `/[locale]/app/ops` now expose `diskTrend` with sample count, trend window, available/used deltas, average used bytes per day, projected-full days, warning days, and minimum window hours.
  - Local verification passed: `bash -n scripts/collect_ops_status.sh`, JSON parse for en-US/zh-CN messages, OpenAPI YAML parse, `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (12 files, 50 tests), `pnpm build`, and targeted `ROOMPIRE_E2E_PORT=3106 pnpm e2e --grep "owner reviews ops health and viewer is blocked"` (desktop/mobile, 2 passed).
  - Production deployment from the self-hosted server used Docker Compose only, without SSH: built `roompire-web:latest`, recreated `roompire-web-1`, confirmed Postgres/Redis/web healthy on `127.0.0.1:18300`, ran real-domain smoke for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`, and refreshed the host ops snapshot.
  - Real-domain verification confirmed authenticated `/api/v1/ops/status` returns `diskTrend` with `minimumWindowHours=6`; current trend status is `unknown` because only about 0.3h of samples exist, so short deployment build spikes do not create a projected-full warning. Real browser desktop and mobile checks opened `/en-US/app/ops`, verified the disk trend card, captured `output/playwright/roompire-disk-trend-live-1783304989448.png` and `output/playwright/roompire-disk-trend-live-mobile-1783305010600.png`, and observed zero console errors.
  - Post-deploy housekeeping reclaimed 1.789GB of Docker build cache and left Docker build cache reclaimable at `0B`; final production ops status still has expected warnings for `disk_high_usage`, `docker_reclaimable_high`, and `backup_offsite_disabled`.
...
- 2026-07-06 Database branch CI triggers:
  ...
  - This is a repository-management change only; no production container or database deployment is required.
- 2026-07-06 Expense proposal filter indexes:
  - Branch: `feat/proposal-filter-indexes`.
  - Added migration `20260706010000_add_expense_proposal_filter_indexes` plus matching Prisma schema index declarations for the expense proposal queue order, status/category/date filters, visible amount filters, creator/member filter, tag lookup, payer lookup, and debtor/creditor share lookup.
  - Local verification passed: `pnpm db:validate`, `pnpm db:migrate`, `pnpm db:generate`, `git diff --check`, `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (12 files, 50 tests), `pnpm build`, and targeted `ROOMPIRE_E2E_PORT=3103 pnpm e2e --grep "dashboard proposal and notification lists paginate"` (desktop/mobile, 2 passed).
  - Production deployment from the self-hosted server used Docker Compose only, without SSH: ran safe housekeeping, built `web` and `migrate`, applied migration `20260706010000_add_expense_proposal_filter_indexes`, recreated `roompire-web-1`, and confirmed Postgres/Redis/web healthy on `127.0.0.1:18300`.
  - Production verification confirmed all 11 new indexes exist in PostgreSQL, real-domain smoke passed for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`, and authenticated real-domain proposal filter API returned HTTP 200 with pagination metadata.
  - Post-deploy housekeeping removed unused `roompire-migrator:latest`, reclaimed 2.54GB of Docker build cache, refreshed ops status, and ended around 9.6GB free / 96% used with expected warnings for disk high usage, Docker reclaimable storage, and disabled offsite backup.
- 2026-07-06 Docker space housekeeping:
  - Branch/commit: `ops/docker-space-housekeeping` / `fc60469 ops: prune unused roompire migrator image`.
  - `scripts/server_housekeeping.sh` now removes unused one-shot Roompire Docker image repositories from `ROOMPIRE_HOUSEKEEPING_ROOMPIRE_EPHEMERAL_IMAGE_REPOSITORIES` (default `roompire-migrator`), while skipping any image that a current or stopped container still references; `ops/systemd/roompire-housekeeping.service` enables the cleanup and the deployment runbook documents the policy.
  - Verification passed: `bash -n scripts/server_housekeeping.sh`, `systemd-analyze verify` for the repo and installed housekeeping units, `git diff --check`, `pnpm format:check`, and housekeeping dry-run output showing `docker image rm roompire-migrator:latest` before any destructive action.
  - Production action was performed directly on the self-hosted server without SSH: installed the updated housekeeping service under `/etc/systemd/system`, reloaded systemd, ran confirmed housekeeping, removed unused `roompire-migrator:latest`, reclaimed 2.539GB of Docker build cache, refreshed ops status, and then started `roompire-housekeeping.service` successfully through systemd.
  - Production validation: Compose still showed Postgres, Redis, and `roompire-web-1` healthy on `127.0.0.1:18300`; real-domain smoke passed for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`; authenticated real-domain `/api/v1/ops/status` returned housekeeping timer/service OK, latest smoke passed, Docker build cache `0`, and root disk around 9.8GB free / 95% used. Expected remaining warnings are `disk_high_usage`, `docker_reclaimable_high`, and `backup_offsite_disabled`.
- 2026-07-06 Expense proposal filters:
  - Branch/commit: `feat/expense-proposal-filters` / `55d333a feat: add expense proposal filters`.
  - Implemented server-side expense proposal filters for `q`, `status`, `categoryId`, `tagId`, `memberUserId`, `from`, `to`, `minAmount`, and `maxAmount`; cursor pagination now validates cursors inside the active filtered window; the dashboard proposal queue exposes URL-backed filters and preserves them through API load-more.
  - Verification passed before deployment: `pnpm format:check`, `git diff --check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (12 files, 50 tests), OpenAPI YAML parse, `pnpm build`, targeted `ROOMPIRE_E2E_PORT=3101 pnpm e2e --grep "dashboard proposal and notification lists paginate"` (desktop/mobile, 2 passed), and full `ROOMPIRE_E2E_PORT=3102 pnpm e2e` (48 passed across Chromium desktop/mobile).
  - Production deployment from the self-hosted server used Docker Compose only, without SSH: cleaned regenerated checkout artifacts for disk headroom, built `web` and `migrate`, ran `prisma migrate deploy` with no pending migrations, recreated `roompire-web-1`, and confirmed Postgres/Redis/web healthy on `127.0.0.1:18300`.
  - Real-domain smoke passed for `https://roompire.aialra.online`: `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`.
  - Live production browser validation through the real site gate opened `/en-US/app`, confirmed the proposal filter form, called the filtered proposal API, submitted URL-backed filters in the UI, verified the filtered empty result state, captured `output/playwright/prod-expense-filters-empty-1783297507485.png`, and observed zero browser console errors. Positive filtering without data creation was not possible because the production household currently has one owner member and no proposals.
  - Ops status was refreshed. The server ended around 3.3GB free / 99% used after the Docker build; root-disk capacity remains the most urgent operational hardening item.
- 2026-07-06 Calendar/task work filters:
  - Branch/commit: `feat/calendar-work-filters` / `45f3fba feat: add calendar work filters`.
  - Implemented server-side calendar event filters for `type`, `status`, `memberUserId`, and recurring-template `categoryId`; task filters for `status`, `priority`, `assignedUserId`, and `categoryId`; URL-backed calendar page filter controls; task create/edit category persistence; localized en-US/zh-CN copy; OpenAPI/docs/test-plan updates.
  - Verification passed before deployment: `pnpm format:check`, `git diff --check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (12 files, 50 tests), `pnpm db:validate`, OpenAPI YAML parse, `pnpm build`, targeted `ROOMPIRE_E2E_PORT=3101 pnpm e2e --grep "member creates calendar work and completes an assigned task"` (desktop/mobile, 2 passed), and full `ROOMPIRE_E2E_PORT=3101 pnpm e2e` (48 passed across Chromium desktop/mobile).
  - Production deployment from the self-hosted server used Docker Compose only, without SSH: cleaned generated artifacts for disk headroom, built `web` and `migrate`, ran `prisma migrate deploy` with no pending migrations, recreated `roompire-web-1`, and confirmed the container healthy on `127.0.0.1:18300`.
  - Real-domain smoke passed for `https://roompire.aialra.online`: `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`.
  - Live production validation through the real site gate created a temporary bill-due event and high-priority categorized task in the active production household, verified event/task filter APIs, opened `/en-US/app/calendar` with filter query parameters, verified the filtered rows in the real browser, captured `output/playwright/roompire-calendar-filters-live.png`, deleted the temporary task/event through the public API, and confirmed zero remaining `Live filter` task/event rows.
  - Ops status was refreshed, post-deploy housekeeping reclaimed 2.538GB of Docker build cache, and the server ended around 5.8GB free / 98% used with Docker build cache at 0B.
- 2026-07-05 Task board views:
  - Branch/commit: `feat/task-board-views` / `64ccf4c feat: add task board views`, pushed to `origin/feat/task-board-views`.
  ...
  - Production deployment from the self-hosted server used Docker Compose only, without SSH: ran safe housekeeping before build, additionally cleaned old browser-workspace generated artifacts to recover headroom, built `web` and `migrate`, ran `prisma migrate deploy` with no pending migrations, recreated `roompire-web-1`, and confirmed the container healthy on `127.0.0.1:18300`.
  - Real-domain smoke passed for `https://roompire.aialra.online`: `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`.
  - Live production validation through the real site gate created a temporary task through the public API in the active production household, opened `/en-US/app/calendar`, verified the task in list, status-board, and due-date calendar views, captured `output/playwright/roompire-task-board-live.png`, and deleted the temporary task through the API.
  - Ops status was refreshed, post-deploy housekeeping reclaimed 2.537GB of Docker build cache, and the server ended around 6.5GB free / 97% used with Docker build cache at 0B.
...
- 2026-07-06 Offsite local filesystem guard:
  - Hardened `scripts/sync_backup_artifacts.sh` local mode to compare `stat` device ids for the backup root and target. Local offsite sync now exits 2 and writes a warning status if the target is on the same filesystem, unless `ROOMPIRE_BACKUP_OFFSITE_ALLOW_SAME_FILESYSTEM=true` is set for a non-offsite drill.
  - Extended `ops/status/backup-offsite.json`, `scripts/collect_ops_status.sh`, `OpsStatusSnapshot`, OpenAPI, localized ops UI, and Playwright fixtures with `sourceDeviceId`, `targetDeviceId`, `sameFilesystem`, and `sameFilesystemAllowed`. The ops summary adds `backup_offsite_same_filesystem` whenever a same-filesystem local sync is recorded, even if drill mode was explicitly allowed.
  - Updated `.env.example`, `.env.production.example`, `ops/systemd/roompire-backup.service`, `docs/11_security_privacy_backup.md`, and `docs/14_deployment_runbook.md` so same-filesystem local targets are rejected by default and only allowed for controlled drills.
  - Verification passed: `bash -n`, i18n JSON parse, temporary same-filesystem reject/allow sync test, temporary ops-status collection test, `git diff --check`, `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, `systemd-analyze verify`, and targeted Playwright desktop/mobile owner ops review plus viewer denial.
  - Installed the updated local `/etc/systemd/system/roompire-backup.service`, ran `systemctl daemon-reload`, and verified the unit. Production gate credentials remained configured in `.env.production` without printing secret values.
  - Production web image was rebuilt and `roompire-web-1` was recreated on the local self-hosted server with the nginx override preserving `127.0.0.1:18300->3000`; Postgres and Redis stayed healthy.
  - Real-domain smoke passed for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`; authenticated `/api/v1/ops/status` returned `backupOffsite.mode=disabled`, `sameFilesystem=null`, `sameFilesystemAllowed=false`, latest smoke `passed`, and expected warnings `disk_high_usage`, `shared_app_storage_attention`, and `backup_offsite_disabled`.
  - Authenticated real-browser smoke opened `https://roompire.aialra.online/en-US/app/ops` on desktop and mobile, verified offsite `Attention`, mode `Not configured`, filesystem check `Unknown`, and no horizontal overflow; screenshots saved to `output/playwright/roompire-ops-offsite-filesystem-live-desktop.png` and `output/playwright/roompire-ops-offsite-filesystem-live-mobile.png`.
- 2026-07-06 Shared app storage skip visibility:
  - Added `ROOMPIRE_SHARED_APP_STORAGE_SKIP_PATHS` to `scripts/collect_shared_app_storage.sh` so known non-Roompire shared app directories that consistently exceed the per-path scan budget can be explicitly skipped instead of producing a partial-scan warning.
  - Extended `ops/status/shared-app-storage.json`, `scripts/collect_ops_status.sh`, `OpsStatusSnapshot`, OpenAPI, localized ops UI, and Playwright fixtures with `skippedPaths`. Skipped paths do not count toward recorded total size, but they are displayed on `/en-US/app/ops` as capacity-planning exceptions.
  - Updated `.env.example`, `.env.production.example`, `ops/systemd/roompire-shared-app-storage.service`, and `docs/14_deployment_runbook.md`; the installed local server unit now skips `/srv/aialra/apps/opencode-turn-engine`.
  - Verification passed: `bash -n`, i18n JSON parse, temporary shared-app skip collector/status test, `git diff --check`, `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, `systemd-analyze verify`, and targeted Playwright desktop/mobile owner ops review plus viewer denial.
  - Ran the real `roompire-shared-app-storage.service`; it exited successfully, wrote `sharedAppStorageInventory.status=ok`, `timedOutPaths=[]`, and `skippedPaths=["/srv/aialra/apps/opencode-turn-engine"]`, then refreshed the main ops snapshot.
  - Production web image was rebuilt and `roompire-web-1` was recreated on the local self-hosted server with the nginx override preserving `127.0.0.1:18300->3000`; Postgres and Redis stayed healthy.
  - Real-domain smoke passed for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`; authenticated `/api/v1/ops/status` showed `sharedAppStorageInventory.status=ok`, the skipped path, latest smoke `passed`, housekeeping `ok`, and expected warnings only `disk_high_usage` plus `backup_offsite_disabled`.
  - Authenticated real-browser smoke opened `https://roompire.aialra.online/en-US/app/ops` on desktop and mobile, verified shared app storage `OK`, skipped path count `1`, skipped `/srv/aialra/apps/opencode-turn-engine`, and no horizontal overflow; screenshots saved to `output/playwright/roompire-ops-shared-storage-skip-live-desktop.png` and `output/playwright/roompire-ops-shared-storage-skip-live-mobile.png`.
  - Post-deploy housekeeping succeeded and refreshed the ops snapshot; root disk ended around 9.2GB free / 96% used, with Docker build cache reduced to about 234MB.
