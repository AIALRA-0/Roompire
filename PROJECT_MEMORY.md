...
2. Decide owner transfer and self-removal semantics; current UI disables self mutation and owner-row mutation while server preserves last-owner guard.
3. Monitor Docker disk usage before the next production image build; builder cache cleanup reclaimed 2.6GB during the audit-chain rollout.
4. Review backup encryption/off-host copy requirements once private production data grows beyond the initial household.
5. Consider a lightweight admin health page for backup timer status, disk headroom, and latest smoke-test result.
6. Design an explicit household clearing policy only if guidance-only netted suggestions should become executable later.
