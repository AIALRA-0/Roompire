...
- Original-currency debt FX policy slice implemented on branch `feat/data-retention-settings`: household settings now expose `ORIGINAL_CURRENCY_DEBT`, cross-currency proposals under that policy keep proposal shares and matured obligations in the entered original currency, store `fxRate=1` with `fxProvider=original-currency-debt`, ignore manual FX input for debt math, and leave only FX-difference adjustment as an unimplemented FX policy.
...
- Production deploy headroom gate slice implemented and deployed on branch `feat/data-retention-settings`: `scripts/check_deploy_headroom.sh` enforces a pre-build root free-space gate shared with `ROOMPIRE_DEPLOY_MIN_AVAILABLE_BYTES`, `collect_ops_status.sh` records `deploymentHeadroom`, `/api/v1/ops/status` and `/[locale]/app/ops` surface deploy-ready/missing bytes, and housekeeping can explicitly clear regenerated Node/pnpm caches plus all Docker builder cache during low-headroom maintenance.
...
| 2026-07-06 | Keep original-currency debts in original currency            | This policy should not synthesize household-currency debt; shares, obligations, balances, suggestions, and settlements stay grouped by the entered currency with a stored neutral rate and provider marker.         |
...
2. Design and implement the remaining FX-difference adjustment policy only after its ledger, settlement, and reporting semantics are explicit.
3. Continue root-disk capacity planning; disk trend, deploy headroom gate, Docker image inventory, safe-cleanup classification, latest confirmed housekeeping cleanup outcome, fast root storage inventory, shared app storage inventory, and old browser-workspace generated-artifact auto-cleanup are live, but repeated production image builds still leave the current server around 7-8GB free / 96-97% used after cleanup. Clearing the high-usage/trend warnings likely needs a larger disk or explicit shared-app/shared-image retention policy.
...
- 2026-07-06 Production deploy headroom gate:
  ...
  - `scripts/collect_ops_status.sh`, `/api/v1/ops/status`, OpenAPI, and the localized ops page now expose `deploymentHeadroom` with required, available, missing bytes, and status. Summary warnings include `deployment_headroom_low` or `deployment_headroom_unknown`.
  - `scripts/server_housekeeping.sh` now supports explicit `ROOMPIRE_HOUSEKEEPING_CLEAN_NODE_CACHES=true` for regenerated Node/pnpm caches and `ROOMPIRE_HOUSEKEEPING_DOCKER_BUILDER_PRUNE_ALL=true` for low-headroom Docker builder-cache cleanup; latest housekeeping status records `nodeCacheCleanupEnabled`.
  - Local verification passed: en-US/zh-CN JSON parse, `bash -n` for changed scripts, OpenAPI YAML parse, `git diff --check`, direct `pnpm --dir apps/web run typecheck`, direct `pnpm --dir apps/web exec eslint .`, `pnpm test` (12 files, 50 tests), `pnpm format:check`, `pnpm build`, and targeted `ROOMPIRE_E2E_PORT=3129 pnpm e2e --grep "owner reviews ops health and viewer is blocked"` on Chromium desktop/mobile.
  - The new gate caught real server pressure before deploy: root free space fell below 6 GiB after build/test work, so confirmed housekeeping with Node/pnpm cache cleanup and all-builder-cache prune recovered headroom before production build.
  - Production deployment was performed directly on the self-hosted server without SSH: `check_deploy_headroom.sh` passed, Docker Compose rebuilt `roompire-web:latest`, recreated `roompire-web-1`, and the web container reached `healthy`.
  - Real-domain smoke passed for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`; authenticated live `/api/v1/ops/status` verified `deploymentHeadroom.status=ok`, `missingBytes=0`, latest smoke `passed`, latest housekeeping `ok`, Docker build cache `0`, container health `ok`, and reminder service `ok`.
  - Authenticated real-browser validation opened `https://roompire.aialra.online/en-US/app/ops` on desktop and mobile, verified deploy headroom OK, missing bytes `0 B`, Node/pnpm cache cleanup shown as enabled, latest smoke passed, container health OK, zero horizontal overflow, and zero console/page errors. Screenshots were saved under `output/playwright/roompire-deploy-headroom-live-*.png`.
  - Final expected ops warnings are `disk_high_usage`, `disk_trend_depleting`, `backup_passphrase_escrow_missing`, and `backup_offsite_disabled`; root ended around 8.3GB free / 96% used after post-deploy cleanup.
- 2026-07-06 Household retention review report:
  - Branch: `feat/data-retention-settings`; commits `4d4d46b feat: add retention review report`, `e324303 fix: stabilize retention review formatting`, `1f44a0c fix: pin retention review timestamps`, and `d63e4c3 fix: render retention review dates deterministically`.
  - Added owner/admin-only `GET /api/v1/households/{householdId}/retention-review`, backed by `apps/web/src/server/households/retention.ts`. It returns `deletionEnabled=false`, current retention days/cutoffs, operational record counts/samples, and attachment file counts/bytes/samples without deleting or marking data.
  - The household settings page now shows a localized retention review summary under Data retention. Dates and bytes use deterministic strings instead of environment-dependent `Intl` output after live production browser testing exposed React hydration error #418 from SSR/browser formatting drift.
  - Updated README, database/API/backlog/acceptance docs, OpenAPI, en-US/zh-CN copy, dashboard model wiring, and Playwright coverage in the existing owner settings flow.
  - Local verification passed: en-US/zh-CN JSON parse, OpenAPI YAML parse, `pnpm typecheck`, `pnpm lint`, `pnpm test` (12 files, 50 tests), `pnpm format:check`, `git diff --check`, `pnpm build`, targeted `ROOMPIRE_E2E_PORT=3123 pnpm e2e --grep "owner updates settings and manages a linked invitee"` on desktop/mobile, and targeted desktop reruns after the hydration fixes.
  - GitHub Actions for final commit `d63e4c3` passed: CI `https://github.com/AIALRA-0/Roompire/actions/runs/28800504145` and E2E `https://github.com/AIALRA-0/Roompire/actions/runs/28800502817`.
  - Production deployment was performed directly on the self-hosted server without SSH: conservative housekeeping recovered build headroom, Docker Compose rebuilt `roompire-web:latest`, recreated `roompire-web-1`, and Postgres/Redis/web were healthy on `127.0.0.1:18300`.
  - Real-domain smoke passed for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`. Authenticated live API verification showed the current household retention review returns `deletionEnabled=false`, null retention policies, and zero due records/files by default.
  - Authenticated real-browser validation created a temporary `Live Retention Review ...` household on `https://roompire.aialra.online`, set retention to `365`/`1095`, verified the retention review API and desktop/mobile settings UI, checked no horizontal overflow and zero console/page errors, saved screenshots under `output/playwright/roompire-retention-review-live-*.png`, and cleaned the temporary household with zero remaining rows.
  - Post-deploy housekeeping reclaimed Docker build cache and refreshed ops status; final smoke still passed. Authenticated ops status showed `containerHealth=ok`, latest smoke `passed`, latest housekeeping `ok`, Docker build cache `0`, root disk about 7.9GB free / 97% used, and expected warnings for high/depleting disk, missing backup passphrase escrow, and disabled offsite backup.
- 2026-07-06 Latest housekeeping cleanup visibility:
  - Branch: `feat/data-retention-settings`; commit `5aaeca7 ops: surface latest housekeeping cleanup`.
  - `scripts/server_housekeeping.sh` now writes `ops/status/latest-housekeeping.json` for confirmed cleanup runs only, recording cleanup status, start/end time, exit code, root free bytes before/after, net reclaimed bytes, and active cleanup modes. Failed confirmed runs record the failing stage; dry runs intentionally do not update the status file.
  - `scripts/collect_ops_status.sh`, `/api/v1/ops/status`, OpenAPI, and the localized ops page now expose `latestHousekeeping`; summary warnings cover missing, failed, unconfirmed, or stale cleanup proof. Updated housekeeping/ops-status systemd templates explicitly set `ROOMPIRE_HOUSEKEEPING_STATUS_FILE`.
  - Local verification passed: `bash -n scripts/server_housekeeping.sh scripts/collect_ops_status.sh`, en-US/zh-CN JSON parse, OpenAPI YAML parse, `systemd-analyze verify`, controlled housekeeping/collector temp-file checks, `pnpm typecheck`, `pnpm lint`, `pnpm test` (12 files, 50 tests), `pnpm format:check`, `git diff --check`, `pnpm build`, and targeted `ROOMPIRE_E2E_PORT=3122 pnpm e2e --grep "owner reviews ops health and viewer is blocked"` on Chromium desktop/mobile.
  - GitHub Actions for `5aaeca7` passed: CI `https://github.com/AIALRA-0/Roompire/actions/runs/28795862272` and E2E `https://github.com/AIALRA-0/Roompire/actions/runs/28795862261`.
  - Production deployment was performed directly on the self-hosted server without SSH: pre-build cleanup raised root headroom from about 3.7GB free / 99% used to about 8.5GB free / 96% used, updated `roompire-housekeeping.service` and `roompire-ops-status.service` in `/etc/systemd/system`, rebuilt `roompire-web:latest`, recreated `roompire-web-1`, and confirmed web/Postgres/Redis healthy on `127.0.0.1:18300`.
  - Post-build `roompire-housekeeping.service` succeeded with `ExecMainStatus=0`, wrote `latestHousekeeping.status=ok`, `cleanupConfirmed=true`, and `reclaimedBytes=2116542464`; authenticated real-domain `/api/v1/ops/status` showed latest housekeeping OK, container health OK, latest smoke passed, backup freshness OK, root disk around 8.7GB free / 96% used, and expected remaining warnings for high disk usage, depleting disk trend, missing backup passphrase escrow, and disabled offsite backup.
  - Real-domain smoke passed for `/en-US`, `/api/v1/health`, and `/manifest.webmanifest`. Authenticated real-browser validation opened `https://roompire.aialra.online/en-US/app/ops` on desktop and mobile, verified the latest cleanup section shows OK, confirmed cleanup enabled, net reclaimed around `+2 GB`, and `ops/status/latest-housekeeping.json`, found zero horizontal overflow and no browser console/page errors, and saved screenshots under `output/playwright/roompire-housekeeping-live-*.png`.
