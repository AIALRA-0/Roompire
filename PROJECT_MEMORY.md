...
2. Monitor Docker disk usage before the next production image build; builder cache cleanup is still necessary because root disk remains tight.
3. Review backup encryption/off-host copy requirements once private production data grows beyond the initial household.
4. Consider a lightweight admin health page for backup timer status, disk headroom, and latest smoke-test result.
5. Design an explicit household clearing policy only if guidance-only netted suggestions should become executable later.
