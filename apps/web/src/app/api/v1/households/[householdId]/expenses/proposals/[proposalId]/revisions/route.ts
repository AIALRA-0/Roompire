...
import { reviseExpenseProposalForHousehold } from "@/server/expenses/service";
...
async function readJsonBody(request: NextRequest) {
  try {
    ...
    throw validationError("Expense proposal revision input is invalid.");
  }
}
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const response = await runIdempotentMutation({
      ...
      householdId,
      method: "POST",
      routeKey: `/households/${householdId}/expenses/proposals/${proposalId}/revisions`,
      requestBody: body,
      handler: async () => {
        const proposal = await reviseExpenseProposalForHousehold(
          user.id,
          householdId,
          proposalId,
          body,
