...
import { createExpenseProposalFromTaskForHousehold } from "@/server/expenses/service";
...
async function readJsonBody(request: NextRequest) {
  try {
    ...
    throw validationError("Task expense proposal input is invalid.");
  }
}
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const idempotencyKey = requireIdempotencyKey(
      ...
      "task expense proposal creation",
    );
    ...
    const response = await runIdempotentMutation({
      ...
      householdId,
      method: "POST",
      routeKey: `/households/${householdId}/tasks/${taskId}/create-expense-proposal`,
      requestBody: body,
      handler: async () => {
        const proposal = await createExpenseProposalFromTaskForHousehold(
          user.id,
          householdId,
          taskId,
          body,
