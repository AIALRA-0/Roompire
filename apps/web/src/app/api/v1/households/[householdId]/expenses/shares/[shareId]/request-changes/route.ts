...
async function readJsonBody(request: NextRequest) {
  try {
    ...
    throw validationError("Expense share change request input is invalid.");
  }
}
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const response = await runIdempotentMutation({
      ...
      householdId,
      method: "POST",
      routeKey: `/households/${householdId}/expenses/shares/${shareId}/request-changes`,
      requestBody: body,
      handler: async () => {
        const proposal = await requestChangesExpenseShareForHousehold(
          user.id,
          householdId,
          shareId,
          body,
