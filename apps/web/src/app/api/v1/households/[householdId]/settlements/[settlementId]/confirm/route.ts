...
import { confirmSettlementForHousehold } from "@/server/settlements/service";
...
async function readJsonBodyOrEmpty(request: NextRequest) {
  ...
  try {
    ...
    throw validationError("Settlement confirmation input is invalid.");
  }
}
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const response = await runIdempotentMutation({
      ...
      householdId,
      method: "POST",
      routeKey: `/households/${householdId}/settlements/${settlementId}/confirm`,
      requestBody: body,
      handler: async () => {
        const settlement = await confirmSettlementForHousehold(user.id, householdId, settlementId);
