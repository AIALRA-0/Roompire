...
Passphrase escrow is tracked as metadata only. After the passphrase has been copied into an external secret-management workflow that is not only this server, `scripts/write_backup_passphrase_escrow_status.sh` records method, optional custodian, and verification timestamp in `ops/status/backup-passphrase-escrow.json`; it never reads, writes, prints, or stores the passphrase. The authenticated ops dashboard warns while encrypted backups are enabled but escrow proof is missing or older than the configured freshness window.
