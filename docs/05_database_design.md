...
5. FX fields must be non-null when original currency differs from settlement currency under `LOCK_AT_EXPENSE_DATE`, `MANUAL_RATE_WITH_APPROVAL`, or `FX_DIFFERENCE_ADJUSTMENT`; manual-policy proposals must record `fx_provider = manual-entry`. Under `ORIGINAL_CURRENCY_DEBT`, proposal settlement currency matches original currency and the copied FX lock records `fx_rate = 1` with `fx_provider = original-currency-debt`. FX difference corrections are append-only `FX_ADJUSTMENT` ledger transactions with newly created debt obligations.
